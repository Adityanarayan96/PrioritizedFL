...
#         del w_accumulate
        ...
        data_test = create_test_data(pr_dict_users, data_train, data_test_w)  #Create test data based on which clients are 
        ...
        for i,j in zip(pr_dict_users.keys(),range(pr_nodes)):
            pr_train_loader_list.append(
                DataLoader(DatasetSplit(data_train, pr_dict_users[i]), batch_size=batch_size_train, shuffle=True, num_workers=2))
        ...
        for i,j in zip(fr_dict_users.keys(),range(fr_nodes)):
            fr_train_loader_list.append(
                DataLoader(DatasetSplit(data_train, fr_dict_users[i]), batch_size=batch_size_train, shuffle=True,num_workers=2))
        ...
        data_train_loader = DataLoader(DatasetSplit(data_train, merged_indices), batch_size=batch_size_eval, num_workers=2)
...
#                         if use_full_batch:
#                             images, labels = sample_fullbatch_fr(0)
#                         else:
#                             images, labels = sample_minibatch_fr(0)
...
#                         stat.collect_stat(seed, num_iter, model, pr_train_loader_list[0], pr_train_loader_list[0], w_global)
...
#                         break
                ...
                for n in pr_node_subset:
                    ...
                    for i in range(0, iters_per_round):
                        ...
                        model.optimizer.zero_grad()
                        output = model.model(images)
                        loss = model.loss_fn(output, labels) + mu/2 * torch.norm(model.get_weight() - w_global)
                        loss.backward()
                        model.optimizer.step()
                    w_tmp = model.get_weight()  # deepcopy is already included here
                    w_tmp -= w_global
...
#                     stat.collect_stat(seed, num_iter, model, data_train_loader, data_text_loader, w_global)
                    last_output = num_iter
