...
def filter_dict_by_labels(fr_dict_users, pr_dict_users, dataset):
    pr_labels = {idx: DatasetSplit(dataset, [idx]).__getitem__(0)[1] for idx in pr_dict_users.keys()}
    filtered_fr_dict_users = {}

    for idx in fr_dict_users.keys():
        fr_label = DatasetSplit(dataset, [idx]).__getitem__(0)[1]
        if fr_label in pr_labels.values():
            filtered_fr_dict_users[idx] = fr_dict_users[idx]

    return filtered_fr_dict_users
...
def fmnist_partition(data_train, n_nodes):
    return general_partition(data_train, n_nodes, 120, 50, 20)

def cifar_partition(data_train, n_nodes):
    return general_partition(data_train, n_nodes, 120, 50, 20)

def emnist_partition(data_train, n_nodes):
    return general_partition(data_train, n_nodes, 600, 60, 72)
...
def include_worker_gradient(worker_index, gradient, global_loss, local_loss, error, error_ltr, iter_fall):
    global_loss_np = global_loss
    local_loss_np = local_loss
    if np.abs(global_loss_np - local_loss_np) < error and iter_fall < iter_stop:
        return True
